...
<template>
  ...
  >
    <div class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-neutral-800">
      ...
      <!-- Cabeçalho -->
      <div class="flex items-center gap-3 border-b border-rose-100 px-6 py-4 dark:border-neutral-800">
        <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-rose-100 dark:bg-rose-900/40">
          <Icon :name="icone" class="text-lg text-rose-800 dark:text-rose-300" />
        </span>
        <span class="flex-1 text-[17px] font-medium text-neutral-900 dark:text-neutral-100">{{ titulo }}</span>
        <button
      ...
      <div class="grid grid-cols-2 gap-4 px-6 py-5">
        <label
          ...
          <input v-else :type="campo.tipo" v-model="modelo[campo.chave]" :placeholder="campo.label" :class="campoClasse" />
        </label>
      </div>

      <!-- Rodapé -->
      <div class="flex justify-end gap-3 border-t border-rose-100 px-6 py-4 dark:border-neutral-800">
        <button
