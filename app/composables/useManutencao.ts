 ...
 const manutencaoEmAndamento = computed (() => 
  ...
  const manutencaoFiltrados = computed(() => {
    if (filtroStatus.value == 'todos')
      return manutencao.value
    return manutencao.value.filter((e) => e.status == filtroStatus.value)
  })
   
  return {manutencao, buscarManutencao, adicionarManutencao, deletarManutencao, totalManutencao, manutencaoAgendadas, manutencaoConcluidos, manutencaoCancelados, manutencaoUrgentes, manutencaoEmAndamento, filtroStatus, manutencaoFiltrados, error, carregando}
