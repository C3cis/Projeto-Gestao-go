...
const custoTotal = computed(() =>
  props.manutencoes.reduce((soma, m) => soma + m.valor, 0)
)
...
<template>
  <article
    ...
    <div class="flex items-start gap-4 border-b border-rose-100 pb-4 dark:border-neutral-700">
      <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-rose-100 dark:bg-rose-900/40">
        <Icon name="tabler:microscope" class="text-2xl 
        text-rose-800 dark:text-rose-300" />
      </span>
      <div class="min-w-0 flex-1">
        <div class="flex flex-wrap items-center gap-2">
          <h2 class="text-lg font-semibold 
          text-neutral-900 dark:text-neutral-100">{{ equipamento.nome }}</h2>
          <span
    ...
    <div class="grid grid-cols-3 gap-3">
      <div class="rounded-xl bg-rose-50 p-3 text-center dark:bg-neutral-800">
        <p class="text-2xl font-bold
         text-rose-800 dark:text-rose-300">{{ quantidade }}</p>
        <p class="text-xs 
        text-neutral-500 dark:text-neutral-400">Manutenções</p>
      </div>

      <div class="rounded-xl bg-rose-50 p-3 text-center dark:bg-neutral-800">
        <p class="text-lg font-bold 
        text-rose-800 dark:text-rose-300">{{ formatarMoeda(custoTotal) }}</p>
    ...
    <div>
      <h3 class="mb-2 text-sm font-semibold text-neutral-700 dark:text-neutral-200">Histórico de manutenções</h3>
      ...
      <ul v-else class="flex flex-col gap-2">
        <li
          ...
          <div class="min-w-0">
            <p class="text-sm font-medium text-neutral-900 dark:text-neutral-100">{{ m.tipoManu }}</p>
          ...
          <div class="flex shrink-0 items-center gap-3">
            ...
            <span class="text-sm font-semibold text-neutral-700 dark:text-neutral-200">{{ formatarMoeda(m.valor) }}</span>
          </div>
        </li>
      </ul>
    </div>
